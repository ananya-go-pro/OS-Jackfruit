...
static volatile sig_atomic_t supervisor_got_sigchld = 0;
...
static void supervisor_signal_handler(int sig)
{
 ...
 void *logging_thread(void *arg)
 {
    ...
    while (bounded_buffer_pop(&ctx->log_buffer, &item) == 0) {
        ...
        cur = ctx->containers;
        while (cur) {
            if (strcmp(cur->id, item.container_id) == 0) {
...
#else
    (void)cfg;
    fprintf(stderr, "child_fn requires Linux container features\n");
    return 1;
#endif
 }
